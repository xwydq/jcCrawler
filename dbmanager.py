...
class CrawlDatabaseManager:
    ...
    def __init__(self, max_num_thread):
        ...
        try:
            cnx = mysql.connector.connect(host=self.SERVER_IP, user='root', password='jctest')
    ...
    ## extract_url
    # initial extract_url
    def init_extract_url(self):
        con = self.cnxpool.get_connection()
        cursor = con.cursor()
        try:
            if time.strftime("%H%M%S", time.localtime()) < '013000':
                user_init = 'update crawl_url SET status_extract="new" where status_extract = "downloading"'
            else:
                user_init = None
            ...
            print 'init_extract_url() ' + err.msg
            return
        finally:
            cursor.close()
            con.close()
    ...
    # get an web catorgy from queue
    def dequeue_extract_url(self):
        con = self.cnxpool.get_connection()
        cursor = con.cursor(dictionary=True)
        try:
            # use select * for update to lock the rows for read
            query = (
                "SELECT `id`, `url_md5`, `url`, `catorgy_url` FROM crawl_url WHERE status_extract='new' ORDER BY `id` ASC LIMIT 1 FOR UPDATE")
            cursor.execute(query)
            if cursor.rowcount is 0:
                return None
            row = cursor.fetchone()
            if row is None:
                return None
            update_query = ("UPDATE crawl_url SET `status_extract`='downloading' WHERE `id`=%d") % (row['id'])
            cursor.execute(update_query)
            con.commit()
            return row
        except mysql.connector.Error as err:
            print 'dequeue_extract_url() ' + err.msg
            return None
        finally:
            cursor.close()
            con.close()

    def finish_extract_url(self, index):
        con = self.cnxpool.get_connection()
        cursor = con.cursor()
        try:
            update_query = ("UPDATE crawl_url SET `status_extract`='done' WHERE `id`=%d") % (index)
            cursor.execute(update_query)
            con.commit()
        except mysql.connector.Error as err:
            # print 'finishUrl() ' + err.msg
